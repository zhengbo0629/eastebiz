...
public interface PackageListDao {
    ...
    //更新包裹信息
    //查询未发货和未确认包裹
    public  List<PackageList>  selectUnShippedAndUnComfirmPackage();
    /*
     * update 按钮   修改
     *   点击按钮先要做一次查询  提交时判断是否被修改为delete*/
    //两个id  这里只需要一个包裹id,公司和护士id已经做完查询了
    public  PackageList  selectOneByid(Integer id);
...
/*
    ...
    public  List<PackageList>  selectAllUnCheckPackage(Integer companyId);
     //*   2.领取用户,改变包裹checkStatus   为正在对单
      //      批量领取用户,改变包裹checkStatus   为正在对单
  public  void updateCheckingStatusBatch(@Param("ids") int[] ids,@Param("checker")String checker);
    //3查看已领取任务  checker就是当前登录用户的名字
   ...
   // public void  updateUnCheckStatus(Integer id);
    ...
    //4更新包裹状态
    // 批量修改包裹状态
    public  void  updateStatusBatch(@Param("status") String status,@Param("ids")int[] ids);
    //5查看已完成任务
   ...
   //4.更新  labelStatus 为sended     Status由InStock 改为 UnShipped
    ...
    public  List<PackageList>  selectAllUnPaiedPackage(Integer userId);
        //修改信用卡信息
    public  void updateCreaditCard(@Param("id") Integer  id,@Param("creditCard") String creaditCard);
