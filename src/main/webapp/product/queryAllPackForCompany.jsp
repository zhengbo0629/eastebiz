    ...
    <script type="text/javascript">
        ...
        $(function () {
            var tableColumns = [
                ...
                {field: 'wareHousePromotPrice', title: '支付状态'},
                {field: 'status', title: '支付信息'},
                {field: 'userName', title: '报单'},
                {field: 'check', title: '对单'},
                {field: 'payer', title: '支付'},
            ...
            $('#queryAllPackForCompany-table').bootstrapTable('destroy');   //动态加载表格之前，先销毁表格


            $('#queryAllPackForCompany-table').bootstrapTable({//表格初始化
                columns: tableColumns,  //表头
                //data: //通过ajax返回的数据
                url:"${pageContext.request.contextPath}/packageList/queryAllEbizCompanyAndPackage",
        ...
        function AddFunctionAlty(value,row,index) {
            return['<button id="update-button" type="button"  onclick="update('+row.id+')" class="btn btn-default-update">修改</button>',
                '<button id="update-button"  type="button"  onclick="Email('+row.id+')" class="btn btn-default-update">Email</button>'].join("")
        }
...
function update(id) {
    $.ajax({
        type:"post",
        async:false,
        url:"${pageContext.request.contextPath}/packageList/queryPackageMessage",
        data:"id="+id,
        success:function () {
            location.href="${pageContext.request.contextPath}/package/editPackageByDoctor.jsp";
        }
    })
}
...
function Email(id) {
    $.ajax({
        type:"post",
        async:false,
        url:"${pageContext.request.contextPath}/packageList/queryPackageEamil",
        data:"id="+id,
        success:function () {
            location.href="${pageContext.request.contextPath}/Eamil/sendEmailToUser.jsp";
        }
    })
        ...
        <div class="row">
            <div class="col-md-10 ">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 ">
                        <table class="table  table-bordered table-striped table-bordered table-hover" id="queryAllPackForCompany-table">
                        </table>
                    </div>
